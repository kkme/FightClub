...

{
...

	{
		...
		string path, filepath(argv0), program;
		...
		if(string::npos == found){
			...
			program = filepath.substr(found+1);

			logFile = path + "/" + program; logFile.append(".log");

		}

		//found = program.find_last_of(".");

		//program = program.substr(0, found);
...

	{
...

		{
			...
			fprintf(stderr, 
				...
				mytime.getdatetime(), pthread_self(), buf, level[flag], file, func, line);
...

		{
			...
			if (NULL == log_file || 
				...
				fprintf(log_file, 
					...
					mytime.getdatetime(), pthread_self(), buf, level[flag], file, func, line) < 0)
